...
This report describes a GPU port of ParaCOSM's search and classification stages. The natural design question is whether to reuse the CPU implementation's depth-first recursion, parallelizing across many independent search trees on the GPU, or to switch to a level-synchronous breadth-first pipeline that is closer to GPU graph traversal kernels~\cite{merrill2015hpcgraph}. We report on both alternatives and summarize three contributions:
