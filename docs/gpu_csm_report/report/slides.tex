...
\begin{frame}{Continuous Subgraph Matching (CSM)}
  ...
  \begin{itemize}
    \item \textbf{应用}：欺诈检测 / 网络监控 / 数据溯源
    \item \textbf{现有 CPU 系统}：GraphFlow、TurboFlux、SymBi、CaLiG、NewSP
    \item \textbf{ParaCOSM (ICPP'25)}：把 5 种算法接到统一执行器，加上两层 CPU 并行
    \begin{itemize}
...
\begin{frame}{GPU 模块}
  \begin{itemize}
    \item \texttt{gpu\_classifier} \hfill mode \texttt{-m gpu} \\
          边的 safe / unsafe 分类
    \item \texttt{gpu\_candidate\_filter} \hfill PFM2 自动 \\
          相邻 query 顶点之间的 candidate 过滤
    \item \texttt{gpu\_search} \hfill mode \texttt{-m gpu\_all} \\
          DFS 风格：每线程持一个栈
    \item \texttt{gpu\_bfs\_search} \hfill modes \texttt{-m gpu\_bfs[\_versioned]} \\
          层同步 BFS，部分匹配集驻留 device memory
  \end{itemize}
  \vfill
  \textbf{Versioned 协议}：在部分匹配上挂时间戳，多条 unsafe 更新共用一次 BFS。
\end{frame}
