...
void process_node(Node* node) {
    ...
    // std out for this process id
    std::cout << "Processing node with data: " << node->data << " on thread: " << std::this_thread::get_id() << std::endl;
}

// 处理节点的具体逻辑
void process_node(Node* node, int thread_id) {
    // 实现具体的节点处理逻辑
    std::cout << "Thread " << thread_id << " processing node with data: " << node->data << std::endl;
}

// 并行DFS函数
void parallel_dfs(Node* node, int depth, int max_depth,
    ThreadPool& pool, ConcurrentVisited& visited, int thread_id) {
    if (!visited.try_visit(node)) return;

    process_node(node, thread_id);

    if (depth < max_depth) {
    // 并行处理子节点
    for (Node* child : node->children) {
    pool.enqueue([child, depth, max_depth, &pool, &visited, thread_id]() {
        parallel_dfs(child, depth + 1, max_depth, pool, visited, thread_id);
    });
    }
    } else {
    // 串行递归处理
