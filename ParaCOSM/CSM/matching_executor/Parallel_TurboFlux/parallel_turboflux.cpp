...
inline void Parallel_TurboFlux::Parallel_FindMatches(uint order_index, uint depth, std::vector<uint>& m, size_t &num_results)
{
    ...
    local_num_result_.assign(NUMT + 2, 0);
    ...
    for (auto& v: DCS_[eidx_[u_min][u]][m[u_min]])
    {
        ...
        else
        {
            ...
            // Share one m snapshot for all items from this outer-loop iteration
...

    {
        ...
        for(size_t i = start; i < end; ++i){ 
            ...
            ProcessVertex(wi.u, wi.u_min, wi.v_idx, m_local, local_num_result_[thread_id], 
                depth + 1, order_index, thread_id);
        }
    }

    for (size_t i = 0; i < local_num_result_.size(); ++i) {
        num_results += local_num_result_[i];
    }
...
void Parallel_TurboFlux::AddEdge(uint v1, uint v2, uint label)
{
    ...
    if (data_.GetVertexLabel(v2) == query_.GetVertexLabel(u2))
    {
            ...
            && d2[u1][v1] == 1 && d2[u2][v2] == 1)
        {
            ...
            Parallel_TurboFlux_FindMatches_ParaCOSM_Kernel(eidx_[u2][u1], 2, m, num_results);
            ...
            && std::find(treeNode_[u2].backwards_.begin(), treeNode_[u2].backwards_.end(), u1) == treeNode_[u2].backwards_.end())
        {
            ...
            Parallel_TurboFlux_FindMatches_ParaCOSM_Kernel(eidx_[std::min(u1, u2)][std::max(u1, u2)], 2, m, num_results);
