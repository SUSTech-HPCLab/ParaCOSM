...
# 查询目录的基础路径
QUERY_BASE_DIR="/home/cc/haibin2/livejournal/unlabel_query/10_self"
...
# 创建结果目录
RESULTS_DIR="/home/cc/haibin2/10_32Thread_results"
mkdir -p "$RESULTS_DIR"
    ...
    if [ -d "$SUB_DIR" ]; then
        ...
        for QUERY_FILE in "$SUB_DIR"/Q_*; do
            if [ -f "$QUERY_FILE" ]; then
                ...
                echo "运行: ./Parallel_CaLiG -d $DATA_GRAPH -s $INSERTION_GRAPH -q $QUERY_FILE"
                
                # 记录开始时间
                START_TIME=$(date +%s.%N)
                
                # 执行命令并捕获输出
                timeout 3600 ./Parallel_CaLiG -d "$DATA_GRAPH" -s "$INSERTION_GRAPH" -q "$QUERY_FILE" > "$RESULTS_DIR/$SUB_DIR_NAME/$QUERY_NAME.log" 2>&1
