...
bool Parrllel_SymBi::process_vertex_layer_local(
    uint u, uint u_min, size_t v_idx, 
    std::vector<uint>& m, 
    std::vector<ExtendableVertex>& extendable, 
    size_t &num_results, 
                                        ...
                                        if (pre_defined_order_.empty()){
                                            ...
                                            // if(auto_tuning){
                                                if((query_.NumVertices() <=7 ) && (query_.NumEdges() > 9) ){
                                                    FindMatches(2, m, extendable, num_results); // FindMatches in single version
                                                }else{
                                                    // Parallel_FindMatches4(2, m, extendable, num_results); //
                                                    // Parallel_Symbi_FindMatches(2, m, extendable, num_results, query_, data_, eidx_, DCS_, d2, n2, 
                                                    // local_vec_visited_local, homomorphism_, NUMTHREAD, vertex_vector, job_queue, treeNode_);
                                                    Parallel_Symbi_FindMatches_ParaCOSM_Kernel(2, m, extendable, num_results);
                                                }
