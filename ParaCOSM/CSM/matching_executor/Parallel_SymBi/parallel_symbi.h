...
private:
    ...
    std::vector<uint> tree_parent_;  // tree_parent_[u] = parent of u in BFS tree (root maps to root)
    std::vector<std::vector<uint>> pre_defined_order_;
    std::vector<std::vector<uint>> pre_defined_backward_nbr_;

    // Versioned matching orders (generated in Preprocessing after BuildDAG)
    ...
    // Slot pool for versioned hot-edge parallelism
...
private:
    ...
    bool process_vertex_layer_local(
        uint u, uint u_min, size_t v_idx, 
        std::vector<uint>& m, 
        std::vector<ExtendableVertex>& extendable, 
        size_t &num_results, 
