...
struct split{
    ...
    ska::flat_hash_map<int, u_set> c_s_nei;
    vec shell;
    vector<u_set> shell_nei;
};

ska::flat_hash_map<int, ska::flat_hash_map<int, split>> matching_order;
...
void generateMO(){
    for(int ui=0; ui<Q_size; ui++) {
        ska::flat_hash_map<int, split> ui_first;
...
bool turnProcess_safe(int v1,int v2) {
...
void turnOnProcess2(int v1, int v2){
    vec temp_v, temp_u;
    G[v1].nei.insert(v2);
    G[v2].nei.insert(v1);
    for(auto& candi : G[v1].cand){
...
void turnOnProcess(int v1, int v2) {
    std::vector<int> temp_v, temp_u;

    // Add edge (v1, v2) and (v2, v1), thread-safe
    #pragma omp critical
    {
        G[v1].nei.insert(v2);
        G[v2].nei.insert(v1);
    ...
    // Collect keys of G[v1].cand to enable parallel iteration
    std::vector<int> cand_keys;
    for (const auto& cand : G[v1].cand) {
        cand_keys.push_back(cand.first);
    }

    // Parallelize over cand_keys
    std::vector<int> to_erase;
    #pragma omp parallel num_threads(4)
    {
        std::vector<int> local_temp_v, local_temp_u;
        ...
        for (size_t i = 0; i < cand_keys.size(); ++i) {
            ...
            // Access G[v1].cand[ui] safely
            for (auto& ui_nei : G[v1].cand[ui]) {
                int uj = ui_nei.first;
                if (G[v2].label == Q[uj].label) {
                    #pragma omp critical
                    {
                    ...
                    if (G[v1].LI[ui]) {
                        ...
                        for (size_t j = 0; j < local_temp_v.size(); ++j) {
                            int v = local_temp_v[j];
                            int u = local_temp_u[j];
                            if (!G[v].LI[u]) {
                                #pragma omp critical
                                {
                                deleteAndCheck(u, v);
        ...
        // Merge local temp vectors if needed
        #pragma omp critical
        {
            temp_v.insert(temp_v.end(), local_temp_v.begin(), local_temp_v.end());
            temp_u.insert(temp_u.end(), local_temp_u.begin(), local_temp_u.end());
        }
    }

    // Delete invalid ui entries
    for (int ui : to_erase) {
        G[v1].cand.erase(ui);
        G[v1].LI.erase(ui);
    }
...
void dumpMatch(ska::flat_hash_map<int, int>& m){
...
bool shellCand(vector<u_set>& result, ska::flat_hash_map<int, int>& m, const vec& s, const vector<u_set>& s_n, vec& used){
    int s_size = s.size();
...
bool notExit(int shell, u_set nei, ska::flat_hash_map<int, int>& m){
    if(nei.size()==1){
        return false;
    }
    int n = *nei.begin();
...
int searchCore_(int th, ska::flat_hash_map<int, int>& m, vec& used, const vec& c, const vector<u_set>& c_n, const vec& s, const vector<u_set>& s_n, ska::flat_hash_map<int, u_set>& c2check){
    int result = 0;
    if(th == c.size()){
    ...
    if(c_n[th].size()>1){
        ...
        for(auto& vth : candidates){
            ...
            result += searchCore_(th+1, m, used, c, c_n, s, s_n, c2check);
            used.pop_back();
        }
    }
    else{
...
int searchMatch(int v1, int v2){
        ...
        if(li.second){
            ...
            for(auto& candi : G[v1].cand[u1]){
                ...
                if(u2>=0 && candi.second.find(v2)!=candi.second.end()){
                    candidate_pairs.push_back({u1, u2});
                }
            }
        }
    }
    ...
    // 使用原子变量存储结果，防止竞态条件
    atomic<int> atomic_result(0);
    int num_res = 0;

    // 并行处理所有候选对
    #pragma omp parallel for num_threads(4) reduction(+:num_res)
    for(int i = 0; i < candidate_pairs.size(); ++i) {
        int u1 = candidate_pairs[i].first;
        int u2 = candidate_pairs[i].second;
        
        ska::flat_hash_map<int, int> matching;
        matching[u1] = v1;
        matching[u2] = v2;
        vec core_v;
...
//             // total_time += (std::chrono::high_resolution_clock::now() - time1)*1000 / CLOCKS_PER_SEC;
//             // time = clock();
...
//             turnOnProcess(v1, v2);
//             update_time += ( std::chrono::high_resolution_clock::now() - start_time)*1000 / CLOCKS_PER_SEC;
//             time = clock();
...
void updateAndMatching() {
    ...
    for (int t = 0; t < update.size(); t += 2) {
        ...
        if(v1<0){
            ...
            search_time += duration_cast<milliseconds>(search_end - search_start).count();
            // microseconds
...
int main(int argc, char** argv){
    ...
    updateAndMatching();
    // Parallel_batch_updateAndMatchingMoreDetect();
