...
#if __has_include("utils/CLI11.hpp")
#include "utils/CLI11.hpp"
#include "utils/globals.h"
#include "utils/types.h"
#else
#include "matching_executor/Parallel_NewSP/utils/CLI11.hpp"
#include "matching_executor/Parallel_NewSP/utils/globals.h"
#include "matching_executor/Parallel_NewSP/utils/types.h"
#endif

// Compatibility: if top-level CSM headers were picked (which define My_Get_Time)
// provide MY_Get_Time alias expected by Parallel_NewSP sources.
#ifndef MY_Get_Time
#ifdef My_Get_Time
#define MY_Get_Time() My_Get_Time()
#endif
#endif
...
// Use timing macros from included utils/types.h
...
int main(int argc, char *argv[])
{
    ...
    Print_Time("Load Graphs Time: ", start);
    ...
    auto IncrementalFun = [&data_graph, &mm, &num_v_updates, &num_e_updates, &start]()
    {
        while (!data_graph.updates_.empty())
        {
            ...
            {
                mm->Safe_Update(insert.id1, insert.id2, insert.label);

                if(!mm->safe_detect(insert.id1, insert.id2, insert.label, pos)){
        ...
        for(size_t idx = 0; idx < data_graph.update_vec_.size(); idx++){
    ...
    std::chrono::nanoseconds>(MY_Get_Time() - start).count()/(float)1000 /(float)1000 );
