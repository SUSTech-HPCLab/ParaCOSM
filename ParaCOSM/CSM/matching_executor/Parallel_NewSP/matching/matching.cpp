...
bool matching::safe_detect(uint v1, uint v2, uint label, searchType type){
    return false;
}

void matching::Safe_Update(uint v1, uint v2, uint label){
    data_.AddEdge(v1, v2, label);
    data_.indexUpdate(v1, v2, label, true);
}

void matching::Safe_Update_remove(uint v1, uint v2){
    auto label = std::get<2>(data_.GetEdgeLabel(v1, v2));
    data_.RemoveEdge(v1, v2);
    data_.indexUpdate(v1, v2, label, false);
}
