    ...
    print_prep, print_enum, homo)
{
    ...
    intersection_parallelResult.resize(Thread_MAX);

    visited_parallel.resize(Thread_MAX);
    match_parallel.resize(Thread_MAX);
    matchCandidate_parallel.resize(Thread_MAX);
...
void CSMPP::AddEdge(uint v1, uint v2, uint label)
{
    ...
    // if(!safe_detect(v1, v2, label, pos)){
...
void CSMPP::Safe_Update(uint v1, uint v2, uint label){
    this->data_.AddEdge(v1, v2, label);
    this->data_.indexUpdate(v1, v2, label, true);
}
...
void CSMPP::Safe_Update_remove(uint v1, uint v2){
    uint label = std::get<2>(this->data_.GetEdgeLabel(v1, v2));
    this->data_.RemoveEdge(v1, v2);
    this->data_.indexUpdate(v1, v2, label, false);
}
...
/**
 * @description: subrgaph search safe detect
 * @param {uint} v1
 * @param {uint} v2
 * @param {uint} label
 * @param {searchType} type
 * @return {*}
 */
bool CSMPP::safe_detect(uint v1, uint v2, uint label, searchType type){
...
void CSMPP::localMatchVertex(std::vector<int>& match, std::vector<bool>& visited, uint vertex, uint pos){
    match[pos] = vertex;
    visited[vertex] = true;
}

// 线程本地移除匹配顶点函数
void CSMPP::localPopVertex(std::vector<int>& match, std::vector<bool>& visited, uint vertex, uint pos){
    match[pos] = UNMATCHED;
    visited[vertex] = false;
}

void CSMPP::localPopVertex(std::vector<int>& match,  std::vector<std::vector<uint>>& matchCandidate, uint pos){
    match[pos] = UNMATCHED;
    matchCandidate[pos].clear();
}
 ...
 * @return {*}
 */
void CSMPP::searchVertex_local(uint queryIndex, uint edgeIndex, searchType type, uint depth){
    ...
    if(depth != cacheStatu)
    {
        ...
        if(desItem.size() == 0 && freezeIndex.size() == 0)
        {
            ...
            finish = useAndFinsh;
            if(currentSearchVertexType == freeVertex) // go parallel at here
            {
                for(auto & i : this->matchCandidate[cacheStatu])
                {
                    if(this->visited_[i])continue;
                    this->matchVertex(i,depth);
                    this->searchVertex_local(queryIndex, edgeIndex, type, depth + 1);
                ...
                {   // first search, then do other things!
    ...
    if(!freezeIndex.empty()){
        ...
        while(this->headRecord[depth][0] >= 0){
            ...
            for(int i = 0; i < freezeIndex.size(); i++){
                this->match[freezeIndex[i].first] = -1;
            }
...
void CSMPP::matchVertexlocal(uint vertex, uint pos, std::vector<int> & match, std::vector<bool>& visited_ ){
    match[pos] = vertex;
    visited_[vertex] = true;
}
void CSMPP::matchVertexAll(uint vertex, uint pos){
    for(int i =0; i < Thread_MAX; i++){
        match_parallel[i][pos] = vertex;
        visited_parallel[i][vertex] = true;
...
void CSMPP::matchVertexlocal(std::vector<uint> & Candidate, uint pos, std::vector<int> & match,
     std::vector<std::vector<std::seed_seq::result_type>>& matchCandidate){
    match[pos] = -1;
    matchCandidate[pos] = std::move(Candidate);
}
...
//    match[pos] = -1;
//    matchCandidate[pos] = std::move(Candidate);
    matchCandidate_parallel[thread_id][pos] = std::move(Candidate);
    match_parallel[thread_id][pos] = -1;
}
...
void CSMPP::popVertex_local(uint vertex, uint pos, std::vector<int> & match, std::vector<bool>& visited_ ){
    match[pos] = UNMATCHED;
    visited_[vertex] = false;
}
void CSMPP::popVertexAll(uint vertex, uint pos){
    // this->match[pos] = UNMATCHED;
    // this->visited_[vertex] = false;
    for(int i =0; i < Thread_MAX; i++){
        match_parallel[i][pos] = UNMATCHED;
        visited_parallel[i][vertex] = false;
    }
}
...
void CSMPP::popVertex_local(uint pos, std::vector<int> & match, std::vector<std::vector<std::seed_seq::result_type>>& matchCandidate ){
    match[pos] = UNMATCHED;
    matchCandidate[pos].clear();
}

void CSMPP::popVertex_local(uint pos, std::vector<int> & match, std::vector<std::vector<std::seed_seq::result_type>>& matchCandidate, size_t thread_id ){
    // match[pos] = UNMATCHED;
    match_parallel[thread_id][pos] = UNMATCHED;
    // matchCandidate[pos].clear();
    matchCandidate_parallel[thread_id][pos].clear();
}

void CSMPP::popVertexAll(uint pos){
    // this->match[pos] = UNMATCHED;
    // this->matchCandidate[pos].clear();
    for(int i =0; i < Thread_MAX; i++){
        match_parallel[i][pos] = UNMATCHED;
        matchCandidate_parallel[i][pos].clear();
    }
}
 ...
 * @return {*}
 */
bool CSMPP::getIntersection_Parallel(uint vertex, uint queryVertexLabel, const std::vector<uint> & needToIntersection, int depth, uint queryIndex, uint queryVertex, uint elabel){
    std::vector<uint>result;
    const auto & vNeighbors = this->data_.GetNeighbors(vertex);
    const auto & vNeighborsLabels = this->data_.GetNeighborLabels(vertex);
    if(!needToIntersection.empty()){
...
void CSMPP::searchVertex_local2(uint queryIndex, uint edgeIndex, searchType type, uint depth, size_t thread_id){
    auto & queryGraph = this->queryVec[queryIndex];
    const uint cacheStatu = queryGraph.getCacheStatu(edgeIndex, depth);
    const auto & matchOrder = queryGraph.GetMatchOrder(edgeIndex);
    const uint queryVexter = matchOrder[depth];
        ...
        if(desItem.size() == 0 && freezeIndex.size() == 0)
        {
            ...
            if(currentSearchVertexType == freeVertex) // go parallel at here
            {
                for(auto & i : this->matchCandidate[cacheStatu])
                {
                    if(this->visited_[i])continue;
                    this->matchVertex(i,depth);
                    this->searchVertex_local2(queryIndex, edgeIndex, type, depth + 1, thread_id);
                    this->popVertex(i, depth);
                }
            }
            else{
