...
TIME_RE = re.compile(r"^#Time:\s+([0-9.]+)\s+ms\s*$", re.MULTILINE)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Compare serial and parallel NewSP on every query file in a directory.")
    parser.add_argument("--query-dir", type=Path, required=True, help="Directory that contains query files.")
    ...
    parser.add_argument("--serial-exe", type=Path, default=subproject_root / "build/csm_serial")
    parser.add_argument("--parallel-exe", type=Path, default=subproject_root / "build/csm")
    parser.add_argument("--serial-threads", type=int, default=1)
    parser.add_argument("--parallel-threads", type=int, default=8)
    parser.add_argument("--time-limit", type=int, default=60)
    parser.add_argument("--initial-time-limit", type=int, default=60)
    parser.add_argument("--run-timeout", type=int, default=180)
    parser.add_argument("--report-initial", choices=("on", "off", "1", "0"), default="on")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=repo_root / "logs_txt/compare_newsp_batch",
        help="Directory for per-query logs and comparison summary.",
    )
...
def run_one(exe: Path, threads: int, query_path: Path, args: argparse.Namespace, log_path: Path) -> RunStats:
    command = [
        str(exe.resolve()),
        "-d",
    ...
    return RunStats(
        ...
        incremental_ms=extract_first_float(TIME_RE, combined_output),
...
def compare_stats(query_name: str, query_path: Path, serial: RunStats, parallel: RunStats) -> CompareResult:
    same_counts = (
...
def main() -> int:
    ...
    if not args.query_dir.is_dir():
        ...
        return 2
    if not args.serial_exe.is_file() or not args.parallel_exe.is_file():
        print("Serial or parallel executable not found.", file=sys.stderr)
        return 2
    ...
    for query_path in queries:
        ...
        print(f"[RUN ] {query_name} serial")
        serial_stats = run_one(args.serial_exe, args.serial_threads, query_path, args, serial_log)
        print(f"[RUN ] {query_name} parallel")
        parallel_stats = run_one(args.parallel_exe, args.parallel_threads, query_path, args, parallel_log)

        result = compare_stats(query_name, query_path, serial_stats, parallel_stats)
    ...
    timeout_count = sum(1 for result in results if result.match_status.startswith("timeout"))
    fail_count = sum(1 for result in results if result.match_status == "fail")

    print()
    print(f"Summary: total={len(results)} pass={pass_count} timeout_related={timeout_count} fail={fail_count}")
