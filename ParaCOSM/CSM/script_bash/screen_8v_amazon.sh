#!/bin/bash
# =============================================================================
# Screen 8v queries on amazon: parallel first, then serial if feasible
# Amazon loads in ~2s so we can run 200 queries much faster.
# =============================================================================
set -euo pipefail

CSM_BIN="./build/bin/csm"
DATA_GRAPH="${HOME}/haibin/paracosm_data/6/data_graph/data.graph"
INSERT_GRAPH="${HOME}/haibin/paracosm_data/6/data_graph/insertion.graph"

QUERY_DIR="${1:?Usage: $0 <query_dir> [output_dir]}"
OUTPUT_DIR="${2:-logs_txt/screen_8v_amazon}"
mkdir -p "${OUTPUT_DIR}"

THREADS=8
PAR_TIME_LIMIT=${PAR_TIME_LIMIT:-60}
PAR_TIMEOUT=${PAR_TIMEOUT:-120}
SER_TIME_LIMIT=${SER_TIME_LIMIT:-180}
SER_TIMEOUT=${SER_TIMEOUT:-300}
...
echo ""
echo "================================================================"
echo " Screen 8v on AMAZON: parallel(${THREADS}T) first, then serial"
