...
OUTPUT_DIR=logs_txt/livejournal/Turboflux

# Time limit (in seconds)
TIME_LIMIT=180

# Graph files
DATA_GRAPH=/home/cc/haibin2/livejournal/30/data_graph/data.graph
INSERT_GRAPH=/home/cc/haibin2/livejournal/30/data_graph/insertion.graph
...
for SUFFIX in "${SUFFIXES[@]}"; do
    # Dataset
    DATA_SET=Turboflux/${SUFFIX}



    # Output file
    OUTPUT=${ALGORITHM}_Turboflux_${SUFFIX}_P2.txt

    # Define configurations for sparse, dense, tree
    CONFIGS=(
        "sparse ${DIR}/livejournal/${SUFFIX}_self/sparse"
        "dense ${DIR}/livejournal/${SUFFIX}_self/dense"
        "tree ${DIR}/livejournal/${SUFFIX}_self/tree"
    )
    ...
    for CONFIG in "${CONFIGS[@]}"; do
        ...
        for i in $(seq 1 99); do
            ...
            if [ "$TYPE" = "tree" ]; then
                ...
                timeout 900 ./build/csm -a ${ALGORITHM} -d ${DATA_GRAPH} -u ${INSERT_GRAPH} -q ${QUERY_GRAPH} \
