...
// Time counting
#define My_Get_Time() std::chrono::high_resolution_clock::now()

#define My_Duration(start) std::chrono::duration_cast<\
    std::chrono::microseconds>(My_Get_Time() - start).count()/(float)1000

#define Print_Time(str, start) std::cout << str << My_Duration(start) << \
    "ms" << std::endl
